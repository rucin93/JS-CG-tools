  ...
  if (test_regex(A_regex)) {
    ...
    outer_loop: for (let first = 8; first < 12; ++first) {
      for (let second = 12; second < 16; ++second) {
        const reg = new RegExp(`uD[${first.toString(16).toUpperCase()}${second.toString(16).toUpperCase()}]`, "g")
        if (test_regex(reg)) {
          ret.push(reg)
          break outer_loop
        }
...
function do_pack(to_pack, regex) {
  ...
  for (let j = 0; j < 65536; ++j) {
    ...
    if (!char_map.has(q) || char_map.get(q)[1] > char_cost) char_map.set(q, [c, char_cost])
  ...
  for (let i = 0; i < 1024; ++i) {
    const unpack1 = unescape(("%u" + (0xd800 + i).toString(16).toUpperCase()).replace(regex, ""))
    if (to_pack.includes(unpack1)) high_surrogate_unpack.set(i, unpack1)

    const unpack2 = unescape(("%u" + (0xdc00 + i).toString(16).toUpperCase()).replace(regex, ""))
  ...
  for (const [i_high, high_unpack] of high_surrogate_unpack) {
    for (const [i_low, low_unpack] of low_surrogate_unpack) {
      ...
      if (!char_map.has(q) || char_map.get(q)[1] > 1)
        char_map.set(q, [String.fromCodePoint(65536 + i_high * 1024 + i_low), 1])
      max_length = Math.max(max_length, q.length)
    }
  }

  //Using dynamic programming to find shortest packing
  const state = [{ str: "", cost: 0 }]
    ...
    if (state[0] !== undefined) {
      for (let l = 1; l <= max_length; l++) {
        ...
        if (k !== undefined) {
          ...
          if (cost1 < (state[l]?.cost ?? Number.POSITIVE_INFINITY)) state[l] = { str: str1, cost: cost1 }
        }
      }
    }
    state.shift()
  ...
  return {
    length: (state[0]?.cost ?? Number.POSITIVE_INFINITY) + regex.toString().length + 37,
    packed_string: state[0]?.str,
    packed_program: "eval(unescape(escape`" + state[0]?.str + "`.replace(" + regex + ",``)))",
    regex: regex,
  }
}

function display_pack(pack) {
  if (pack.length === Number.POSITIVE_INFINITY) {
  ...
  for (const c of evaled_string) {
    const decoded = unescape(escape(c).replace(pack.regex, ``))
      .replace(/[\x00-\x20]/gu, " ")
      .replace(/[^\x20-\x7E]/gu, "?")
    source += decoded
...
function run_tests() {
  const tests = [
    ...
    [64, unescape("%u85000%0%u95000%0%uA5000%0%uB5000%0%uC5000%0%D5000%0%E5000%0%F5000%0")],
    [54, unescape("%B000%C000%E000%F000%8000%9000%B000%C000%E000")],
    [49, "\\😀"],
    [56, "5%A+-6%B+-7%C+-8%D+-9%E+-0%F"],
    [49, "XY%1%2%3%4%5%6%7%8"],
    [Number.POSITIVE_INFINITY, unescape("%uD800%uDC00%uD900%uDD00%uDA00%uDE00%uDB00%uDF00")],
  ]

  const all_regexes = [/uD./g, /u\D*/g]
  for (let first = 8; first < 12; ++first) {
    all_regexes.push(new RegExp(`uD*${first.toString(16).toUpperCase()}?`, "g"))
    all_regexes.push(new RegExp(`uD+${first.toString(16).toUpperCase()}?`, "g"))
    for (let second = 12; second < 16; ++second)
      all_regexes.push(new RegExp(`uD[${first.toString(16).toUpperCase()}${second.toString(16).toUpperCase()}]`, "g"))
  }
  ...
  for (const [expected_length, to_pack] of tests) {
    ...
    const pack_results_auto = possible_best_regexes(to_pack).map((r) => do_pack(to_pack, r))
    const best_pack_auto = Math.min(...pack_results_auto.map((x) => x.length))

    if (best_pack_auto !== best_pack || best_pack !== expected_length) {
      console.log("Failed test: auto =", best_pack_auto, "best =", best_pack, "expected =", expected_length)
...
export function twoOnePlus(to_pack, verbose) {
  try {
    let best_pack = { length: Number.POSITIVE_INFINITY }
